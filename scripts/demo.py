...
transaction_hash = requests.get(f"{api}/create_transaction?amount={transaction_data['amount']}&data={json.dumps(transaction_data['data'])}&gas_limit={transaction_data['gas_limit']}&sender={transaction_data['sender']}&recipient={transaction_data['recipient']}&private_key={transaction_data['private_key']}").json()['data']
